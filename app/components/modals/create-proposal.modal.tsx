...
export const CreateProposalModal = ({
  ...
  const onSubmit: SubmitHandler<ICreateProposalInput> = async (data) => {
    try {
      ...
      const tokenSymbol = isTestaPlanet ? "TESTA" : "TLM";
      const tokenContract = isTestaPlanet ? "token.worlds" : "alien.worlds";
      ...
      handleShowFeedbackModal(true, {
        message:
          "Proposal created successfully, please wait for the arbiter to approve it.",
        type: "success",
      });
    } catch (error: any) {
      handleShowFeedbackModal(true, {
